...
// 移除重复的CORS配置，只保留一个完整的配置
app.use(cors({
    origin: '*',
    methods: ['GET', 'POST', 'OPTIONS'],
    allowedHeaders: ['Content-Type', 'Authorization', 'X-Requested-With', 'Accept', 'Accept-Encoding', 'Accept-Language'],
    credentials: true,
...
function callGemini(messages, res, cancelTokenSource, originalRequest) {
    return new Promise((resolve, reject) => {
        ...
        const makeRequest = async () => {
            try {
                // 检查 PROXY_URL2 是否存在
                if (!process.env.PROXY_URL2) {
                    logger.error({ 
                        logType: 'gemini_request', 
                        message: 'PROXY_URL2 环境变量未设置'
                    });
                    throw new Error('PROXY_URL2 not configured');
                }
...
app.listen(PROXY_PORT, () => {
    logger.info({ 
        logType: 'system', 
        message: `Hybrid AI proxy server started on port ${PROXY_PORT}`,
        config: {
            cors: true,
            port: PROXY_PORT,
